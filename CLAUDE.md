...
## Architecture
- **App.tsx**: Root component. Holds band state (4 EQ bands), selected band, header with centered PresetSelector, EQ display, tabbed band controls, status bar.
...
- **BandControls.tsx**: Controls for selected band. Band type selector buttons (LC, PK, HC, BH, BL — LS/HS removed), enable/bypass toggle, 3 ControlKnob instances (Frequency, Gain, Bandwidth).
...
## Band Types
`BandType = 'lowcut' | 'lowshelf' | 'peaking' | 'highshelf' | 'highcut' | 'brickwalllow' | 'brickwallhigh'`
...
6. **Master band removed** — User preference. Only 4 EQ bands remain (no master/final stage).
